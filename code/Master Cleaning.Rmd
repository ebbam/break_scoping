...
owid_elec <- owid_all %>% 
  select(country, year, population, gdp, electricity_generation, electricity_demand, 
         ...
         contains("fossil"), contains("share_elec"), contains("electricity")) %>% mutate(country = ifelse(country %in% names(getcountry), unname(getcountry[country]), country)) %>% 
...
dd_20_21 <- readRDS(here("data/out/elec_emissions_panel_clean_22.RDS")) %>% filter(country %in% sel & year >= 2020) %>% select(year, country, HDD16dailybypop, CDD18dailybypop) %>% rename(hdd = HDD16dailybypop, cdd = CDD18dailybypop) %>% pivot_longer(cols = !c(country, year), names_to = 'cat', values_to = 'n')
...
# Sanity check
data_dd_full %>% select(1:114) %>% identical(full_gdp)
...
readRDS(here("data/out/elec_emissions_panel_clean_22.RDS")) %>% identical(data_dd_full)
...
ets_price <- read.xlsx(here("data/raw/icap_eu_ets_price.xlsx")) %>% 
  mutate(Date = dmy(Date), year = lubridate::year(Date))
  
annual_ets_price <- ets_price %>% 
  group_by(year) %>% 
  summarise(ets_price = mean(EURO, na.rm = TRUE))

ggplot() + 
  geom_line(data = annual_ets_price, aes(x = year, y = ets_price), na.rm = TRUE)

ets_test <- left_join(data_dd_full, annual_ets_price, by = 'year')

#saveRDS(ets_test, here("data/out/elec_emissions_panel_clean_22.RDS"))
