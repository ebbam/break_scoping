...
test_diff(gdp_c, data, c('country', 'year')) 

w_gdp <- left_join(data, gdp_c, by = c('country', 'year')) 

readRDS(here("data/out/elec_emissions_panel_clean_22.RDS")) %>% all.equal(w_gdp)
...
readRDS(here("data/out/elec_emissions_panel_clean_22.RDS")) %>% 
  ...
  all.equal(filter(full_gdp[1:105], country != "Canada"))
...
# HDD and CDD Supplement: to be added
```{r}

supp_hdd <- read_xlsx(here("data/raw/hdd_cdd_supp.xlsx"), sheet = "HDD_15") %>% 
  pivot_longer(!Country, names_to = "year", values_to = "HDD") %>% 
  rename(country = Country)

supp_cdd <- read_xlsx(here("data/raw/hdd_cdd_supp.xlsx"), sheet = "CDD_21.1") %>% 
  pivot_longer(!Country, names_to = "year", values_to = "CDD") %>% 
  rename(country = Country)

supp_weather <- full_join(supp_hdd, supp_cdd, by = c('country', 'year')) %>% 
  filter(country %in% c("United States", "Canada", "Switzerland", "Japan", "United Kingdom", "New Zealand", "Australia") & year >= 1990)

#saveRDS(supp_weather, here("data/temp/supp_weather_slim.RDS"))
