...
ov_disp %>%
  filter(mod_name == "Standard Controls w. Temp" & ets_spec %in% c("w.o ETS", "w. ETS") & !(dep %in% c("Low Carbon Share", "Renewables Share", "Fossil Share"))) %>% 
...
ov_disp %>%
   filter(ar == 0 & !(dep %in% c("Low Carbon Share", "Renewables Share", "Fossil Share")) & !grepl("price", model)) %>%
   dplyr::group_split(dep) %>%
      purrr::iwalk(.,~{
        ind <- unique(.x$dep)
        cat('##### ', ind, '{.tabset}   \n')
